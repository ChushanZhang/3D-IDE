...
from llava.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX, OBJECT_TOKEN, GROUND_TOKEN
...
class ModelArguments:
    ...
    # New configuration for 3D PE and depth auxiliary task
    skip_3d_pe_fusion: bool = field(default=True, metadata={"help": "Skip 3D positional encoding fusion (new default)"})
    use_depth_auxiliary_task: bool = field(default=False, metadata={"help": "Use depth prediction as auxiliary task during training"})
    dpt_checkpoint_path: Optional[str] = field(default="VGGT_checkpoints/model.pt", metadata={"help": "Path to pretrained DPT checkpoint"})
    weak_geometric_validator: bool = field(default=True, metadata={"help": "Train DPT from scratch (do not load pretrained DPT head from vggt)"})
...
class DataArguments:
    ...
    # Cross-view projection constraint settings
    use_cross_view: bool = field(default=True, metadata={"help": "Enable cross-view consistency loss (frame_pairs)."})
    cross_view_neighbor_range: int = field(default=2, metadata={"help": "Number of neighboring frames used for cross-view projection constraint."})
    cross_view_neighbor_direction: str = field(default="forward", metadata={"help": "Direction for cross-view pairing: forward/backward/bidirectional."})
    # Feature 3D distillation toggle
    use_feature_3d_distillation: bool = field(default=True, metadata={"help": "Enable 3D feature distillation loss (loads feature_3d from .npz files)"})
    # Dataset group selection
    dataset_group: Optional[str] = field(
        default=None,
        metadata={
            "help": "Select dataset group for training. Options: 'understanding' (scanrefer/multi3drefer/scan2cap), 'understanding_refer' (scanrefer only), 'understanding_basic' (scanrefer/scan2cap), 'reasoning' (scanqa/sqa3d), None (all datasets). Default: None (train on all datasets)"
        }
    )
    # 🧪 Debug: Limit training samples
    max_train_samples: Optional[int] = field(
        default=None,
        metadata={"help": "For debugging: limit the number of training samples. Default: None (use all samples)"}
    )
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    mm_vision_adapter_lr: Optional[float] = None
    dpt_lr: Optional[float] = None
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    #     input_id, target = [], []
    #     is_grounding = False  # Track if this sample is a grounding task
    ...
    #             target += [IGNORE_INDEX] * len(encode_id)
    #         else:
    #             # For assistant responses: mask entire response if it's a grounding task
    #             # Grounding samples should not contribute to CE Loss at all
    #             # (only grounding head is trained, text generation is not used in inference)
    #             if content is not None and "<ground" in content:
    #                 # Mask entire assistant response for grounding tasks
    #                 target += [IGNORE_INDEX] * len(encode_id)
    #                 is_grounding = True  # Mark this sample as grounding
    #             else:
    #                 # Normal text-only samples: train language modeling
    #                 target += encode_id
    ...
    #     assert len(input_id) == len(target), f"{len(input_id)} != {len(target)}"
    #     for idx, encode_id in enumerate(input_id):
    #         # Only unmask special tokens for non-grounding samples
    #         # Grounding samples should have 100% tokens masked (Language Loss = 0)
    #         if encode_id in unmask_tokens_idx and not is_grounding:
    #             target[idx] = encode_id
    #         if encode_id == image_token_index:
    #             input_id[idx] = IMAGE_TOKEN_INDEX
    #     input_ids.append(input_id)
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        # Configure cross-view settings (projection constraints)
        if getattr(data_args, "use_cross_view", None) is not None:
            self.video_processor.use_cross_view = data_args.use_cross_view
        if getattr(data_args, "cross_view_neighbor_range", None) is not None:
            self.video_processor.cross_view_neighbor_range = data_args.cross_view_neighbor_range
            self.video_processor.cross_view_top_k = data_args.cross_view_neighbor_range
        if getattr(data_args, "cross_view_neighbor_direction", None) is not None:
            self.video_processor.cross_view_neighbor_direction = data_args.cross_view_neighbor_direction
        ...
        if "{" in data_path and "}" in data_path:
            ...
            with open(data_path, "r") as file:
                ...
                # 🆕 Filter datasets based on dataset_group parameter
                if data_args.dataset_group is not None:
                    # Define dataset groupings
                    DATASET_GROUPS = {
                        "understanding": ["scanrefer", "multi3drefer", "scan2cap"],
                        "understanding_refer": ["scanrefer"],
                        "understanding_basic": ["scanrefer", "scan2cap"],  # Without multi3drefer
                        "reasoning": ["scanqa", "sqa3d"]
                    }

                    if data_args.dataset_group not in DATASET_GROUPS:
                        raise ValueError(f"Invalid dataset_group: {data_args.dataset_group}. Must be one of {list(DATASET_GROUPS.keys())} or None")

                    selected_datasets = DATASET_GROUPS[data_args.dataset_group]
                    rank0_print(f"🎯 Dataset group '{data_args.dataset_group}' selected. Only loading: {selected_datasets}")

                    # Filter datasets based on the group
                    filtered_datasets = []
                    for dataset in datasets:
                        json_path = dataset.get("json_path")
                        # Extract dataset name from path (e.g., "scanrefer" from "data/processed/scanrefer_vg_train_llava_style.json")
                        dataset_name = None
                        for ds_name in ["scanrefer", "multi3drefer", "scan2cap", "scanqa", "sqa3d"]:
                            if ds_name in json_path.lower():
                                dataset_name = ds_name
                                break

                        if dataset_name in selected_datasets:
                            filtered_datasets.append(dataset)
                            rank0_print(f"  ✓ Including dataset: {json_path}")
                        else:
                            rank0_print(f"  ✗ Skipping dataset: {json_path} (not in '{data_args.dataset_group}' group)")

                    datasets = filtered_datasets
                    rank0_print(f"📊 Total datasets after filtering: {len(datasets)}")
                ...
                for dataset in datasets:
                    ...
                    # 🧪 Debug: Apply max_train_samples PER dataset (not globally)
                    if data_args.max_train_samples is not None and data_args.max_train_samples > 0:
                        original_len = len(cur_data_dict)
                        cur_data_dict = cur_data_dict[:data_args.max_train_samples]
                        rank0_print(f"🧪 DEBUG: Limited {json_path} from {original_len} to {len(cur_data_dict)} samples")
        ...
        rank0_print(f"Loaded {len(self.list_data_dict)} total samples from {data_path}")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            # 🆕 Restore box_input for GROUND_TOKEN feature (SCAN2CAP)
            # box_input: [x, y, z, w, h, l] - target object's center and dimensions
            # Keep as list here, will be converted to tensor in merge_video_dict
            box_input = self.list_data_dict[i].get("box_input", None)
            ...
            try:
                video_dict = self.video_processor.process_3d_video(
                    ...
                    load_feature_3d=self.data_args.use_feature_3d_distillation,  # toggle controlled by config
                )
                image = video_dict.pop("images")
                video_size = video_dict.pop("video_size")
                video_dict["box_input"] = box_input  # 🆕 Used for GROUND_TOKEN feature
                video_dict["scene_id"] = video_file  # DEBUG: track which scene
                ...
                # 🆕 GROUND_TOKEN: Add <ground> token AFTER coordinates when box_input exists (SCAN2CAP)
                # This allows <ground> to "see" the coordinates via causal attention
                # Prompt: "...located at [x, y, z] <ground> in detail..."
                # Unified: both SCAN2CAP and grounding use <ground> token
                if box_input is not None:
                    conv_value = sources[0]["conversations"][0]["value"]
                    # Construct coordinate string (matches prompt format: 2 decimal places)
                    # box_input is a list [x, y, z, w, h, l]
                    coord_str = f"[{box_input[0]:.2f}, {box_input[1]:.2f}, {box_input[2]:.2f}]"
                    # Replace coordinate with coordinate + space + <ground>
                    conv_value = conv_value.replace(coord_str, coord_str + " " + GROUND_TOKEN)
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            ...
            if "video_dict" in instances[0]:
                batch["video_dict"] = merge_video_dict([instance["video_dict"] for instance in instances], training=True)
        ...
        # # CONSTANT PATH for torch.compile: Always enable object proposals
        # # Text-only samples use dummy box_labels=[-1] and dummy video_dict
        # batch["box_labels"] = [
        #     inst["box_label"] if "box_label" in inst and len(inst.get("box_label", [])) > 0
        #     else [-1]  # Text-only: dummy zero-target
        #     for inst in instances
        # ]
        # batch["use_object_proposals"] = True

        # # For text-only batches: create minimal dummy video_dict
        # if "video_dict" not in batch:
        #     batch["video_dict"] = {"objects": [[torch.zeros(6)]]}
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        [
            ...
            model_args.skip_3d_pe_fusion is not None,
            model_args.use_depth_auxiliary_task is not None,
        ]
    ):
    ...
    # Add 3D PE and depth auxiliary task configurations to config before model init
    overwrite_config["skip_3d_pe_fusion"] = model_args.skip_3d_pe_fusion
    overwrite_config["use_depth_auxiliary_task"] = model_args.use_depth_auxiliary_task
    overwrite_config["dpt_checkpoint_path"] = model_args.dpt_checkpoint_path
    overwrite_config["weak_geometric_validator"] = model_args.weak_geometric_validator
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
        ...
        elif "qwen" in model_args.model_name_or_path.lower(): # default qwen
            if "moe" in model_args.model_name_or_path.lower() or "A14B" in model_args.model_name_or_path:
                ...
                # vision_tower ----> SigLIP-SO400M (0.4 B)
                # mm_projector ----> 0.017B 
                # LLM-Qwen-7B ----> layers: 6.53B + lm_head: 0.54B + embed_tokens: 0.54B ~= 7.6B
...
def train(attn_implementation=None):
    ...
    # 🚀 Enable TF32 for faster matmul on Ampere+ GPUs (A100, RTX 3090/4090, etc.)
    # TF32 uses 19-bit precision (vs 23-bit FP32) for ~8x speedup with minimal accuracy loss
    # Compatible with BF16 training - they work at different levels (storage vs computation)
    # if torch.cuda.is_available():
    #     torch.backends.cuda.matmul.allow_tf32 = True
    #     torch.backends.cudnn.allow_tf32 = True
    #     if torch.cuda.get_device_capability()[0] >= 8:
    #         rank0_print("✓ TF32 enabled for Ampere+ GPU (expected 5-10% speedup)")
    #     else:
    #         rank0_print("✓ TF32 setting applied (but GPU architecture < Ampere, no effect)")
    ...
    # 🆕 Add <object> token for SCAN2CAP (like <ground> for grounding)
    # Always enabled - used when box_input exists in data
    # add_tokens_and_resize_embeddings(
    #     ["<coord>"],
    #     tokenizer=tokenizer,
    #     model=model,
    ...
    if model_args.vision_tower is not None:
        ...
        # print('model weight 111:', model.get_vision_tower().vision_tower.vision_model.encoder.layers[7].adapter.adapter_linear_down.weight.shape)
        # for name, param in model.named_parameters():
        #     if 'adapter_linear_up' in name:
        #         torch.nn.init.zeros_(param)
        #     elif 'adapter' in name:
        #         torch.nn.init.kaiming_normal_(param)
        #         print('initialized adapter weight', name, param.clone().cpu())
        ...
        model.config.mm_vision_adapter_lr = training_args.mm_vision_adapter_lr
