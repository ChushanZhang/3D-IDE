...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 🔍 GC MONITOR: Track Python garbage collection pauses
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class GCMonitor:
    """Monitor Python GC pauses that may cause training slowdowns."""
    ...
    def _gc_callback(self, phase, info):
        if phase == 'start':
            self.gc_start_time = _time.perf_counter()
        elif phase == 'stop' and self.gc_start_time is not None:
            gc_duration_ms = (_time.perf_counter() - self.gc_start_time) * 1000
            if gc_duration_ms > self.threshold_ms:
                self.gc_times.append(gc_duration_ms)
                # Get rank safely
                try:
                    ...
                    rank = 0
                print(f"   🗑️  [Rank {rank}] GC PAUSE: {gc_duration_ms:.1f}ms (gen={info.get('generation', '?')})")
            self.gc_start_time = None

    def get_recent_gc_time(self, clear=True):
        """Get total GC time since last call."""
        total = sum(self.gc_times)
        if clear:
            self.gc_times = []
        return total

# Global GC monitor instance
_gc_monitor = GCMonitor(threshold_ms=500)  # Report GC pauses > 500ms
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_projector_lr is not None:
                ...
                special_list.append('mm_projector')
            if self.args.mm_vision_adapter_lr is not None:
                lr_mapper["vision_adapter"] = self.args.mm_vision_adapter_lr
                special_list.append('vision_adapter')
            if self.args.mm_vision_tower_lr is not None:
                lr_mapper["vision_tower"] = self.args.mm_vision_tower_lr
                special_list.append('vision_tower')
            if self.args.dpt_lr is not None:
                lr_mapper["dpt_module"] = self.args.dpt_lr
                special_list.append('dpt_module')
            if len(lr_mapper) > 0:
                ...
                used_list = []
                for module_keyword in special_list:
                    lr = lr_mapper[module_keyword]
                    module_parameters = [name for name, _ in opt_model.named_parameters() if module_keyword in name]
                    
                    # if self.args.local_rank == 0:
                    #     print('all', module_keyword, ':', module_parameters, lr, used_list)

                    weight_decay_param_list = []
                    weight_non_decay_param_list = []
                    # No changes needed, as the code is already correct and consistent with the context.
                    name_list = []
                    for n, p in opt_model.named_parameters():
                        flag = False
                        for used_keyword in used_list:
                            if used_keyword in n:
                                flag = True
                                break
                        if flag:
                            continue 
                        if n in decay_parameters and n in module_parameters and p.requires_grad:
                            weight_decay_param_list.append(p)
                            name_list.append(n)
                        elif n not in decay_parameters and n in module_parameters and p.requires_grad:
                            weight_non_decay_param_list.append(p)
                            name_list.append(n)
                    if self.args.local_rank == 0:
                        print(module_keyword, ':', name_list, lr)
                    optimizer_grouped_parameters.extend(
                        [
                            {
                                "params": weight_decay_param_list,
                                "weight_decay": self.args.weight_decay,
                                "lr": lr,
                            },
                            {
                                "params": weight_non_decay_param_list,
                                "weight_decay": 0.0,
                                "lr": lr,
                            },
                        ]
                    )
                    used_list.append(module_keyword)
                    # optimizer_grouped_parameters.extend(
                    #     [
                    #         {
            ...
            # Debug: log per-group LR mapping to confirm "auto" values resolved correctly (rank 0 only).
            if getattr(self.args, "local_rank", 0) == 0:
                print("[DEBUG] Optimizer param groups (lr, weight_decay, count):")
                for idx, group in enumerate(self.optimizer.param_groups):
                    lr_val = group.get("lr", self.args.learning_rate)
                    wd_val = group.get("weight_decay", 0.0)
                    param_count = sum(p.numel() for p in group["params"])
                    print(f"  group {idx}: lr={lr_val} wd={wd_val} params={param_count}")
            # print('self.optimizer created:', self.optimizer.param_groups)
