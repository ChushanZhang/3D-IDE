...
def eval_model(scene_groups, args):
    ...
    Each scene's video data is loaded once and reused for all questions in that scene.
    """
    # Model
    disable_torch_init()
    ...
    for video_id, scene_questions in scene_groups:
        # Load video data ONCE per scene
...
if __name__ == "__main__":
    ...
    # Group questions by scene (video_id) to avoid redundant video loading
    scene_groups = defaultdict(list)
    for q in questions:
        scene_groups[q["video"]].append(q)

    # Sort scenes by question count (descending) for better GPU load balancing
    ...
    print(f"Questions per scene: min={min(len(qs) for _, qs in scene_list)}, "
          f"max={max(len(qs) for _, qs in scene_list)}, "
          f"avg={len(questions)/len(scene_list):.1f}")
