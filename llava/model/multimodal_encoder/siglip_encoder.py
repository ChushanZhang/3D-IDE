...
def voxelize_and_average(feat_xyz, features):
    ...
    for b in range(batch_size):
        # Pull data for batch b.
        indices_b = voxel_indices[b] - min_indices[b, 0]  # shift indices to start from zero
...
def map_voxel_features_to_points(feat_xyz, voxel_features):
    ...
    for b in range(batch_size):
        ...
        # Per-point voxel feature lookup.
...
class CLIP_3D_Reuse(nn.Module):
    ...
    def forward(
        ...
        """
        Voxelize the multi-view video features by their coords, process the voxel volume, then map results back to the per-frame layout.

        Args:
        - video_tensor: (V, L, C) per-frame feature tensor.
        - coord_info: (V, H, W, 3) per-frame coords (L = H*W).
        - xxx_function: callable applied to the voxelized features.

        Returns:
        - processed_video_tensor: (V, L, C) feature tensor after processing.
        """
        V, L, C = video_tensor.size()
        H, W = coord_info.size(1), coord_info.size(2)
        assert L == H * W, "L should be equal to H * W"

        # Reshape coords from (V, H, W, 3) to (V, L, 3).
        ...
        # Discretize coords into voxel indices.
        voxel_indices = torch.floor(coord_all).long()  # (V*L, 3)

        # Shift voxel indices to start at zero.
        min_indices = voxel_indices.min(dim=0, keepdim=True)[0]  # (1, 3)
        voxel_indices -= min_indices  # voxel indices now start at zero

        # Compute the global voxel grid shape.
        max_indices = voxel_indices.max(dim=0, keepdim=True)[0]  # (1, 3)
        grid_shape = (max_indices - min_indices + 1).squeeze(0)  # (3,)

        # Compute strides for the flattened voxel index.
        ...
        # Process the populated voxel features in bulk.
        ...
        # Map processed voxel features back to per-point.
        processed_features_all = voxel_features_processed[indices_flat]  # (V*L, C)

        # Reshape back to (V, L, C).
...
class SigLipVisionTower(nn.Module):
    def __init__(self, vision_tower, vision_tower_cfg, delay_load=False):
        ...
        # self.intermediate_layer_idx = [6, 12, 18, 24]
        self.intermediate_layer_idx = [6, 12, 18, -1]
    ...
    def forward(self, images, coords=None):
        if type(images) is list:
            ...
            for image in images:
                ...
                for i in self.intermediate_layer_idx:
                    feature = image_forward_out.hidden_states[i].to(image.dtype)
                    if feature.shape[-2] == 729 + 16:
                        feature = feature[:, 16:, :]
                    assert feature.shape[-2] == 729
                    multi_level_features.append(feature)
                image_features.append(multi_level_features)
        else:
            ...
            multi_level_features = [image_forward_outs.hidden_states[i] for i in self.intermediate_layer_idx]
            image_features = []
            for feature in multi_level_features:
                image_feature = feature.to(images.dtype)
                if image_feature.shape[-2] == 729 + 16: 
                    image_feature = image_feature[:,16:,:]
