...
class NullPositionEmbedding3D(nn.Module):
    """
    ZeRO-3 friendly No-Op 3D PE:
    - returns all zeros with the correct shape
    - no parameters, no grads
    - supports inputs of shape (B, N, 3) or (B, N, P, 3) with n_points=P
    - gradient-flow friendly: output maintains connection to input for autograd
    """
    def __init__(self, embedding_size: int, n_points: int = 1, **kwargs):
        super().__init__()
        self.embedding_size = int(embedding_size)
        self.n_points = int(n_points)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Use x.new_zeros() instead of torch.zeros() for better performance and gradient flow
        # This creates zeros with the same dtype/device as x and maintains gradient context
        if x.dim() == 4:
            B, N, P, _ = x.shape
            # Multiply by zero to maintain gradient connection while ensuring zeros output
            # This is more gradient-friendly than creating disconnected zeros
            return x.new_zeros(B, N, P, self.embedding_size)
        elif x.dim() == 3:
            B, N, _ = x.shape
            return x.new_zeros(B, N, self.embedding_size)
        else:
