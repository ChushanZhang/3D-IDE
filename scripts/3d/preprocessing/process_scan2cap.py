...
def main(args):
    ...
    for split in ["train", "val"]:
        ...
        for i, item in enumerate(tqdm(data)):
            ...
            # Use actual coordinates as text (following VG-LLM approach)
            # For training, use ground truth box; for val, use predicted box
            coord_box = gt_box if split == 'train' else select_box
            if coord_box is not None:
                coord_text = f"[{coord_box[0]:.2f}, {coord_box[1]:.2f}, {coord_box[2]:.2f}]"
            else:
                coord_text = "[0.00, 0.00, 0.00]"  # fallback
            ...
            new_item = {
                ...
                "conversations": [
                    {
                        "value": f"<image> Carefully watch the video and describe the object located at {coord_text} in detail.",
                        "from": "human",
                    },
                    {
                        "value": f"{desc}",
